...
class TrainOptions():
    def __init__(self):
        ...
        self.parser.add_argument("--exstyle_path", type=str, default=None,
                                 help="path to the saved extrinsic style codes")
        self.parser.add_argument("--instyle_path", type=str, default=None,
                                 help="path to the saved intrinsic style codes")
        ...
        self.parser.add_argument("--model_name", type=str, default='refined_exstyle_code.npy',
                                 help="name to save the refined extrinsic style codes")
...
if __name__ == "__main__":
    ...
    for ii in range(0, len(files), args.batch):
        ...
        for i in pbar:
            ...
            latent = generator.generator.style(
        ...
        with torch.no_grad():
            ...
            for j in range(imgs.shape[0]):
                vis = torchvision.utils.make_grid(
                    torch.cat([imgs[j:j + 1], img_gen0[j:j + 1], img_gen[j:j + 1].detach()], dim=0), 3, 1)
                save_image(torch.clamp(vis.cpu(), -1, 1),
                           os.path.join("./log/%s/refine_exstyle/" % (args.style), batchfiles[j]))
