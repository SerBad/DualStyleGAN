...
from util import data_sampler, requires_grad, accumulate, sample_data, d_logistic_loss, d_r1_loss, g_nonsaturating_loss, \
    g_path_regularize, make_noise, mixing_noise, set_grad_none
...
class TrainOptions():
    def __init__(self):
        ...
        self.parser.add_argument("--n_sample", type=int, default=9,
...
def pretrain(args, loader, generator, discriminator, g_optim, d_optim, g_ema, encoder, vggloss, device, inject_index=5,
    ...
    with torch.no_grad():
        ...
        target_img, _ = generator(sample_zs, None, input_is_latent=False, z_plus_latent=False,
                                  inject_index=inject_index, use_res=False)
    ...
    for idx in pbar:
        ...
        with torch.no_grad():
            ...
            target_img, _ = generator(real_zs, None, input_is_latent=False, z_plus_latent=False,
                                      inject_index=inject_index, use_res=False)
            target_img = target_img.detach()
            # g(z2)
...
if __name__ == "__main__":
    ...
    pretrain(args, loader, generator, discriminator, g_optim, d_optim, g_ema, encoder, vggloss, device, inject_index=7,
             savemodel=False)
    args.iter = full_iter // 10
    pretrain(args, loader, generator, discriminator, g_optim, d_optim, g_ema, encoder, vggloss, device, inject_index=6,
             savemodel=False)
    args.iter = full_iter
